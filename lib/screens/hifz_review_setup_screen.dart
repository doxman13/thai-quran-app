...
class _HifzReviewSetupScreenState extends State<HifzReviewSetupScreen>
  ...
  Future<void> _checkForResumableSession() async {
    ...
    for (var s in allSessions) {
      if (s.sessionType == HifzSessionType.review) {
        snap = s;
        break;
      }
    }
    ...
    final resume = await showDialog<bool>(
      ...
      builder: (ctx) {
        ...
        final typeLabel = isThai ? 'ทบทวนฮิฟซ์ (Review)' : 'Review Mode';
...
class _ByPageTabState extends State<_ByPageTab> {
  ...
  Future<void> _openVisualPicker() async {
    if (widget.quranRepository == null) return;
    final result = await Navigator.push<HifzMushafRangePickerResult>(
      context,
      MaterialPageRoute(
        builder: (_) => HifzMushafRangePickerScreen(
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(
      ...
      children: [
        ...
        if (widget.quranRepository != null) ...[
          const SizedBox(height: 16),
          OutlinedButton.icon(
            onPressed: _openVisualPicker,
            icon: const Icon(Icons.auto_stories_rounded, size: 20),
            label: Text(
              isThai
                  ? 'เลือกหน้าจากมุศฮัฟ (Visual Picker)'
                  : 'Select Page from Mushaf (Visual)',
              style: const TextStyle(fontWeight: FontWeight.bold),
            ),
            style: OutlinedButton.styleFrom(
              minimumSize: const Size.fromHeight(48),
