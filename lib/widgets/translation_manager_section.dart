...
class TranslationManagerSection extends StatefulWidget {
  ...
  const TranslationManagerSection({Key? key, required this.colors})
    : super(key: key);
...
class _TranslationManagerSectionState extends State<TranslationManagerSection> {
  ...
  final List<Map<String, dynamic>> _availableTranslations = [
    ...
    {
      'id': 230,
      'name': 'Society of Institutes',
      'author': 'Society of Institutes',
      'language': 'Thai',
    },
    {
      'id': 51,
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Text(
              context.tr('active_translations'),
            ...
            _buildLegacyCheckbox(settings, 'thai_v3', context.tr('thai_v3')),
            _buildLegacyCheckbox(settings, 'thai_v2', context.tr('thai_v2')),
            _buildLegacyCheckbox(settings, 'english', context.tr('english')),

            const Divider(),
            Text(
              context.tr('download_more'),
              style: GoogleFonts.inter(
                fontWeight: FontWeight.w600,
                color: widget.colors.textStrong,
              ),
            ...
            ..._availableTranslations.map((t) {
              final isDownloaded = transManager.downloadedTranslations.any(
                (dt) => dt['id'] == t['id'],
              );
              ...
              return ListTile(
                ...
                trailing: isDownloaded
                    ? IconButton(
                        ...
                        onPressed: () =>
                            _deleteTranslation(t['id'], transManager, settings),
                      )
                    : (progress != null)
                    ? SizedBox(
                        width: 24,
                ...
                leading: isDownloaded
                    ? Checkbox(
                        ...
                        onChanged: (val) async {
                          if (val == true) {
                            await transManager.loadTranslationIntoCache(
                              t['id'],
                            );
                            ...
                            if (settings.primaryTranslationId == idStr) {
                              if (settings.secondaryTranslationId != null) {
                                ...
                                ScaffoldMessenger.of(context).showSnackBar(
                                  SnackBar(
                                    content: Text(
                                      context.tr('at_least_one_active'),
                                    ),
                                  ),
                                );
                              }
  ...
  Widget _buildLegacyCheckbox(
    ...
    return CheckboxListTile(
      title: Text(
        label,
        style: GoogleFonts.inter(color: widget.colors.textStrong),
      ),
      subtitle: isPrimary
          ? Text(
              context.tr('primary'),
  ...
  Future<void> _downloadTranslation(Map<String, dynamic> t) async {
    ...
    if (mounted) {
      ...
      if (success) {
        Provider.of<TranslationManagerProvider>(
          context,
          listen: false,
        ).refreshDownloadedList();
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text('${t['name']} downloaded!')));
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Failed to download ${t['name']}')),
        );
      }
    }
  }
